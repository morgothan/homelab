...
JELLYFIN_URL     = os.getenv("JELLYFIN_URL",  "http://plex.iot.lan:8096")
    ...
    try:
        ...
        if VLLM_MODEL:
            ctx = (
                ...
                f"- Currently loaded vLLM model on spark.lan: `{VLLM_MODEL}`\n\n"
            ) + ctx
...
_SOURCE_HOWTO = {
    ...
    "vLLM":            "SSH to spark.lan: ~/bin/update-vllm.sh <version> "
    ...
    "DGX Spark":       "SSH to spark.lan: sudo apt update && sudo apt upgrade "
...
def update_howto(*, container: str = "", host: str = "", source_label: str = "") -> str:
    ...
    if host == "nntmux":
        return ("No direct SSH to the nntmux LXC — via pve.lan: sudo pct exec 106 -- "
